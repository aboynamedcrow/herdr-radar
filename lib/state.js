...
const paneLabels = { at: 0, values: new Map() };

async function snapshot() {
  if (config.preferPaneLabels && Date.now() - paneLabels.at >= 5000) {
    const panes = await herdr.panesAsync();
    paneLabels.values = new Map(panes.map((p) => [p.pane_id, p.label]));
    paneLabels.at = Date.now();
  }
    ...
    return [
      {
        ...
        title: hook.apply(
          ...
          vendorTitle(
            ...
            stripVendorPulse(
              said(
                config.preferPaneLabels ? paneLabels.values.get(pane) : '',
...
function clearState(source, pane) {
  ...
  for (const name of require('./details').OWNED) tokens[name] = null;
  for (const name of LOGO_TOKENS) tokens[name] = null;
  for (const state of STATES) {
    tokens[`name_${state}`] = null;
...
function groupBoundaries(entries, parentOf = new Map()) {
  ...
  for (const entry of entries) {
    ...
    if (!tails.has(entry.workspace)) {
      // A child already supplied the family header for its following parent.
      if (parentOf.get(previousWorkspace) !== entry.workspace) heads.add(entry.pane);
      previousWorkspace = entry.workspace;
    }
...
async function writeGroups(source, entries, wsLabels, staleWorkspaces = new Set(), tree = {}) {
  ...
  const { heads, tails } = groupBoundaries(entries, parentOf);
  const order = [];
  const seen = new Set();
  for (const entry of entries) {
  ...
  // Native workspace order can separate a parent from its worktrees. Repeat
  // the parent header at each new block. Never draw a branch under another repo.
  const familyOf = (workspace) => parentOf.get(workspace) ?? workspace;
  const parentHeaders = new Map(orphanRepo);
  const lastChild = new Set();
  const noGap = new Set();
  for (let i = 0; i < order.length; i += 1) {
    const workspace = order[i];
    const parent = parentOf.get(workspace);
    const family = familyOf(workspace);
    if (parent && (i === 0 || familyOf(order[i - 1]) !== family)) {
      parentHeaders.set(workspace, wsLabels.get(parent) ?? parent);
    }
    if (i + 1 < order.length && familyOf(order[i + 1]) === family) {
      noGap.add(workspace);
    }
    if (parent && (i + 1 === order.length || parentOf.get(order[i + 1]) !== parent)) {
      lastChild.add(workspace);
    }
  }
  const results = await Promise.all(
    entries.map((entry) => {
      ...
      const orphan = parentHeaders.has(entry.workspace);
      const corner = parent && !lastChild.has(entry.workspace) ? '├' : '└';
      const branch = orphan ? `${corner}─ ${mark}` : parent ? `${INDENT}${corner}─ ${mark}` : '';
      ...
      return herdr.reportMetadataAsync(entry.pane, source, {
        ...
        group_parent: orphan && name !== null ? parentHeaders.get(entry.workspace) : null,
...
// Native clients choose their own order. Shared headers cannot follow two
// different client sorts, so each entry carries its complete context.
async function writeEntryHeaders(source, entries, wsLabels, staleWorkspaces = new Set(), tree = {}) {
  const results = await Promise.all(
    entries.map((entry) => {
      const parent = tree.parentOf?.get(entry.workspace);
      const repo = parent ? (wsLabels.get(parent) ?? parent) : tree.orphanRepo?.get(entry.workspace);
      const workspace = wsLabels.get(entry.workspace) ?? entry.workspace ?? '';
      const label = repo && repo !== workspace ? `${repo} › ${workspace}` : workspace;
      const stale = staleWorkspaces.has(entry.workspace);
      return herdr.reportMetadataAsync(entry.pane, source, {
        group: stale ? null : label,
        group_stale: stale ? label : null,
        group_parent: null,
