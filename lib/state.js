...
const { logoFor, stateGlyph, blockedFrame } = require('./logos');
...
// The vendors that get a named cell of their own over there. It is a short
// list on purpose: a workspace publishes its whole token set in one patch,
// Herdr refuses a patch over sixteen tokens whole rather than truncating, and
// it says nothing when it does — so a name added here has to be paid for.
// Everyone else shares `other`, which is also the right answer for a brand
// with no hue (lib/palette.js).
const SPACE_VENDORS = ['claude'];
...
function spaceLogoTokens(agents) {
  const out = { space_logo_claude: null, space_logo_other: null };
  const seen = new Set();
  const others = [];
  for (const a of agents) {
    ...
    if (SPACE_VENDORS.includes(a.name)) out[`space_logo_${a.name}`] = label;
