...
function logoCell(token, glyphs, { bold = false, dim = false } = {}) {
  ...
  // No `fg` on the cell itself. A vendor with a hue gets it from a rule above;
  // everything else — a brand that signs in black, an agent we have no mark for
  // — is left alone, and Herdr keeps the contextual default, which is the row's
  // own ink. So those marks follow the terminal's theme (white on a dark panel,
  // black on a light one) instead of wearing a colour we made up for them.
  const base = `token = "${token}", bold = ${bold}, dim = ${dim}`;
...
function sidebarBlock(variant) {
  ...
  // to go.
  // Only vendors with a hue of their own get a named working mark here; the
  // rest share `other`. The roster is deliberately short: a workspace writes
  // its whole token set in one patch, Herdr rejects a patch over sixteen
  // tokens whole rather than truncating, and it says nothing when it does —
  // so every name added here costs a token that has to come from somewhere.
  const spaceMarks = [
