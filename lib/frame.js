...
const { detailTokens } = require('./details');
...
class Frame {
  ...
  async render(now) {
    ...
    // Only our own grouped view guarantees one shared order across clients.
    ...
    const heads = grouped ? state.groupBoundaries(displayEntries, keys.parentOf).heads : new Set();
    ...
    for (const entry of entries) {
      ...
      // native sorting, every entry has a context header and a continuation row.
      ...
      const detailWidth = grouped
        ? config.groupIndentWidth * (1 + Number(keys.nested(entry.workspace))) + corner.length
        : config.groupIndentWidth;
      this.paneJobs(
        entry,
        display,
        { tabs, keys, indent, corner, spinStep, header, detailWidth },
        now,
        deadlines,
        jobs,
      );
  ...
  paneJobs(
    entry,
    display,
    { tabs, keys, indent, corner = '', spinStep, header = '', detailWidth = 0 },
    now,
    deadlines,
    jobs,
  ) {
    ...
    const details = config.detailRows
      ? detailTokens(tabs.get(entry.tab), entry.tokens ?? {}, detailWidth)
      : detailTokens('', {}, 0);
    const key = line
      ? `${display}:${line.mark}:${line.split}:${line.logo}:${line.titlePrefix}:${title}:${JSON.stringify(details)}`
      : '';
    if (this.lastLine.get(pane) !== key && this.writable(`line:${pane}`, now)) {
      // Send the difference, not the whole set: see state.stateTokens.
      const tokens = line ? { ...state.stateTokens(display, line, title), ...details } : null;
  ...
  async groupJobs(entries, displayEntries, viewMode, grouped, wsAgents, workspaces, keys, now, deadlines) {
    ...
    if (state.INDENT) {
      const wrote = grouped
        ...
        : await state.writeEntryHeaders(this.src, entries, workspaces, staleWorkspaces, {
            parentOf: keys.parentOf,
            orphanRepo: keys.orphanRepo,
          });
      ok = wrote.ok;
