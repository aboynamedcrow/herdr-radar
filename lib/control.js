...
const { stateRoot, ensureDir, canonical, digest } = require('./paths');
const { NAME } = require('./identity');

function endpoint() {
  if (process.platform === 'win32') {
    // Per-user so two accounts on one machine never fight over the name.
    return `\\\\.\\pipe\\${NAME}.${os.userInfo().username}.${digest(canonical(stateRoot))}.ctl`;
  }
  const local = path.join(stateRoot, 'control.sock');
  if (Buffer.byteLength(local) < 104) return path.join(ensureDir(stateRoot), 'control.sock');
  // macOS limits Unix socket paths to 104 bytes, including the terminator.
  // A private temp directory keeps long home paths out of the socket name.
  const name = `${NAME}-${process.getuid()}`;
  let dir = path.join(os.tmpdir(), name);
  if (Buffer.byteLength(path.join(dir, `${digest(canonical(stateRoot))}.sock`)) >= 104) dir = path.join('/tmp', name);
  fs.mkdirSync(dir, { recursive: true, mode: 0o700 });
  return path.join(dir, `${digest(canonical(stateRoot))}.sock`);
}
